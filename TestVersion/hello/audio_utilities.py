...
def get_signal(in_file, expected_fs=22050):
    """Load a wav file.
    ...
    if fs != expected_fs:
        print('[*]change sampling rate : ' , expected_fs)
        duration = y.shape[0] / fs

        time_old  = np.linspace(0, duration, y.shape[0])
        time_new  = np.linspace(0, duration, int(y.shape[0] * expected_fs / fs))
        interpolator = interpolate.interp1d(time_old, y.T)

        new_audio = interpolator(time_new).T
        y = np.round(new_audio).astype(y.dtype)
        #raise Exception('Invalid sample rate.')

        print('[*]after samplig rate : ' , fs)
...
def reconstruct_signal_griffin_lim(magnitude_spectrogram, fft_size, hopsamp, iterations=300):
...
def save_audio_to_file(x, sample_rate, outfile='out.wav',outputdir='output'):
